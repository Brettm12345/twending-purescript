module Component.Typeahead where

import Prelude
import Affjax (printError)
import Affjax as AX
import Affjax.ResponseFormat as AR
import Component.Dropdown as D
import Component.Utils (classNames, css, whenElem)
...
component =
  ...
  handleEvent = case _ of
    S.Selected ix -> do
      ...
      for_ st.available \arr ->
        for_ (arr !! ix) \item -> do
          let
            newSelections = item : st.selections
          H.modify_
            _
  ...
  handleAction = case _ of
    Remove item -> do
      st <- H.get
      let
        newSelections = filter (_ /= item) st.selections
  ...
  render st =
    ...
    renderSelections =
      whenElem hasSelections \_ ->
        HH.div
          [ css "Typeahead__selections" ]
          (renderSelectedItem <$> st.selections)
      where
      renderSelectedItem item =
        HH.div
          [ css "Typeahead__item--selected Location" ]
          [ HH.span
              [ css "Location__name" ]
              [ HH.text item.name ]
          , closeButton item
    ...
    renderContainer =
      ...
      renderItems = do
        let
          renderMsg msg = [ HH.span_ [ HH.text msg ] ]
      ...
      renderItem index { name, population } =
        HH.div
          (SS.setItemProps index [ classNames [ base, highlight, "Location" ] ])
          [ HH.span
              [ css "Location__name" ]
              [ HH.text name ]
          , HH.span
              [ css "Location__population" ]
              [ HH.text population ]
          ]
        where
...
searchLocations search = do
  ...
  let
    body = rmap (\r -> r.body) (lmap printError res)
